...
const nextStepIndex = rendered.indexOf('creative-next-step-strip');
const stepFlowIndex = rendered.indexOf('creative-step-flow');
assert.ok(conceptBoardIndex > -1 && conceptBoardIndex < guidanceStackIndex && guidanceStackIndex < nextStepIndex && nextStepIndex < stepFlowIndex);
const canvasBeforeConcept = rendered.slice(rendered.indexOf('creative-canvas-panel'), conceptBoardIndex);
assert.ok(!canvasBeforeConcept.includes('creative-step-flow'));
